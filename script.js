/* ═══════════════════════════════════════════════════════════════════
   GÉNÉRATEUR DE DOSSARDS — Challenge Connecté 2026
   script.js — version refactorisée avec gestion de trame obligatoire
═══════════════════════════════════════════════════════════════════ */

// ─────────────────────────────────────────────
// SECTION 0 — THÈME CLAIR / SOMBRE
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// SECTION 0b — CONFIGURATION DYNAMIQUE (Feature 3)
// ─────────────────────────────────────────────
const CONFIG_KEY = 'challenge2026_config';

const DEFAULT_CONFIG = {
  eventName: 'Challenge Connecté 2026',
  categories: [
    { name: 'Course 5 km',    type: 'Course', distance: '5',    start: 1,    end: 999,  color: '#FF8080' },
    { name: 'Course 10 km',   type: 'Course', distance: '10',   start: 1001, end: 1999, color: '#D93025' },
    { name: 'Course 21,1 km', type: 'Course', distance: '21,1', start: 2001, end: 2999, color: '#7B0B0B' },
    { name: 'Marche 5 km',    type: 'Marche', distance: '5',    start: 3001, end: 3999, color: '#7DD87D' },
    { name: 'Marche 10 km',   type: 'Marche', distance: '10',   start: 4001, end: 4999, color: '#2E8B57' },
    { name: 'Marche 21,1 km', type: 'Marche', distance: '21,1', start: 5001, end: 5999, color: '#1A4F1A' },
  ]
};
...
// Build CATS dynamically from config
function buildCatsFromConfig(config) {
  const cats = {};
  config.categories.forEach(c => {
    const cssClass = 'cat-dyn-' + c.name.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    cats[c.name] = {
      start: c.start,
      end: c.end,
      cssClass: cssClass,
...
function buildAliasesFromConfig(config) {
  const aliases = {};
  config.categories.forEach(c => {
    aliases[c.name.toLowerCase()] = c.name;
    // Also match with (non-classée) suffix for marches
    if (c.type.toLowerCase() === 'marche') {
      aliases[c.name.toLowerCase() + ' (non-classée)'] = c.name;
    }
    // Handle dot vs comma in distance
    const dotName = c.name.replace(',', '.');
    if (dotName !== c.name) {
      aliases[dotName.toLowerCase()] = c.name;
      if (c.type.toLowerCase() === 'marche') {
        aliases[dotName.toLowerCase() + ' (non-classée)'] = c.name;
      }
    }
  });
  return aliases;
...
let CATS = buildCatsFromConfig(currentConfig);
let CAT_ALIASES = buildAliasesFromConfig(currentConfig);

// Inject dynamic CSS for category colors
function injectCategoryStyles() {
  let existing = document.getElementById('dynamic-cat-styles');
  if (existing) existing.remove();

  const style = document.createElement('style');
  style.id = 'dynamic-cat-styles';
  let css = '';
  Object.values(CATS).forEach(cat => {
    css += `.${cat.cssClass} .dossard-badge { background: ${cat.color}; color: ${isLightColor(cat.color) ? 'rgba(0,0,0,0.8)' : 'white'}; }\n`;
  });
  // CSS variables for legend dots
  currentConfig.categories.forEach((c, i) => {
    css += `:root { --cat-color-${i}: ${c.color}; }\n`;
  });
...
function updateHeaderEventName() {
  const el = document.getElementById('headerEventName');
  if (el) {
    const parts = currentConfig.eventName.split(' ');
    // Try to highlight the second word
    if (parts.length >= 2) {
      el.innerHTML = parts[0] + ' <span>' + parts.slice(1).join(' ') + '</span>';
    } else {
      el.textContent = currentConfig.eventName;
    }
  }
  // Also update the stats toolbar title
  const statsTitle = document.querySelector('.stats-toolbar-title');
  if (statsTitle) statsTitle.innerHTML = `📊 Tableau de bord — ${currentConfig.eventName}`;
}

// Rebuild legend dynamically
function rebuildLegend() {
  const legend = document.getElementById('legend');
  if (!legend) return;
  legend.innerHTML = currentConfig.categories.map(c => {
    const cat = CATS[c.name];
    return `<div class="legend-item"><div class="legend-dot" style="background:${c.color}"></div> ${c.name} (${String(c.start).padStart(4,'0')}–${String(c.end).padStart(4,'0')})</div>`;
  }).join('');
}

// Rebuild filter buttons dynamically
function rebuildFilters() {
  const group = document.querySelector('.filter-group');
  if (!group) return;
  group.innerHTML = `<button class="filter-btn active" data-cat="all" onclick="setFilter('all', this)">Tous</button>`;
  currentConfig.categories.forEach(c => {
    group.innerHTML += `<button class="filter-btn" data-cat="${c.name}" onclick="setFilter('${c.name.replace(/'/g,"\\'")}', this)">${c.name}</button>`;
  });
}

// Apply full config
function applyConfig() {
  CATS = buildCatsFromConfig(currentConfig);
  CAT_ALIASES = buildAliasesFromConfig(currentConfig);
  injectCategoryStyles();
  updateHeaderEventName();
  rebuildLegend();
  rebuildFilters();
  // Update CAT_COLS for list rendering
  updateCatCols();
}
...
// ─────────────────────────────────────────────
// CONSTANTES TRAME A4 PAYSAGE
// ─────────────────────────────────────────────
// A4 paysage : 297 × 210 mm → ratio 1.4143
const A4_LANDSCAPE_RATIO     = 297 / 210;   // ~1.4143
const TRAME_RATIO_TOLERANCE  = 0.05;        // ±3,5% accepté

// Résolutions recommandées (pour info utilisateur)
const TRAME_RES_IDEAL = '3508 × 2480 px';  // A4 @ 300 DPI — impression parfaite
const TRAME_RES_MIN   = '1240 × 874 px';   // A4 @ ~107 DPI — aperçu correct
...
function loadCounters() {
  try {
    ...
    for (const cat of Object.keys(CATS)) {
      counters[cat] = saved[cat] ?? CATS[cat].start;
    }
    return counters;
  } catch {
    const c = {};
    for (const cat of Object.keys(CATS)) c[cat] = CATS[cat].start;
    return c;
  }
}
...
document.getElementById('trameUpload').addEventListener('change', function(e) {
  const file = e.target.files[0];
  if (!file) return;
  // Reset input pour permettre un rechargement du même fichier
  this.value = '';

  const reader = new FileReader();
  reader.onload = ev => {
    const img = new Image();
    img.onload = () => validateAndLoadTrame(img, ev.target.result, file.name);
    img.onerror = () => showToast('❌ Impossible de lire l\'image PNG.');
    img.src = ev.target.result;
  };
  reader.readAsDataURL(file);
});

function validateAndLoadTrame(img, dataUrl, filename) {
  const ratio = img.width / img.height;
  const ratioOk = Math.abs(ratio - A4_LANDSCAPE_RATIO) <= TRAME_RATIO_TOLERANCE;

  if (!ratioOk) {
    const expected = A4_LANDSCAPE_RATIO.toFixed(4);
    const got      = ratio.toFixed(4);
    const isPortrait = ratio < 1;
    const hint = isPortrait
      ? ' (ton image semble être en portrait — pivote-la à 90° avant de recharger)'
      : '';
    showToast(
      `⚠️ Ratio incorrect : ${got} (attendu ~${expected} pour A4 paysage).${hint}\n` +
      `La trame a quand même été chargée, vérifie le résultat.`
    );
  }

  trameSrc      = dataUrl;
  trameFilename = filename;
  updateTrameBtn();
  renderGrid();

  const dim = `${img.width} × ${img.height} px`;
  const dpi = Math.round(img.width / 11.69); // 297mm = 11.69 inch
  showToast(`✅ Trame chargée : ${dim} (~${dpi} DPI)`);
}

function updateTrameBtn() {
  const btn   = document.getElementById('btnTrame');
  const icon  = document.getElementById('btnTrameIcon');
  const label = document.getElementById('btnTrameLabel');

  if (trameSrc) {
    btn.classList.add('trame-loaded');
    icon.textContent  = '✅';
    label.textContent = trameFilename
      ? `Trame : ${trameFilename.length > 24 ? trameFilename.slice(0, 22) + '…' : trameFilename}`
      : 'Trame chargée';
  } else {
    btn.classList.remove('trame-loaded');
    icon.textContent  = '🖼️';
    label.textContent = 'Charger la trame';
  }
}

// ═══════════════════════════════════════════════════════════════════
// SECTION 3 — CHARGEMENT DU FICHIER EXCEL / CSV
// ═══════════════════════════════════════════════════════════════════
...
dropZone.addEventListener('click', e => {
  const clickable = [dropZone, ...dropZone.querySelectorAll('.upload-icon, .upload-title, .upload-sub')];
  if (clickable.includes(e.target)) fileInput.click();
});
...
// ═══════════════════════════════════════════════════════════════════
// SECTION 4 — PARSING ET ATTRIBUTION DES NUMÉROS
// ═══════════════════════════════════════════════════════════════════

function normalizeCategory(raw) {
  if (!raw) return null;
  return CAT_ALIASES[raw.trim().toLowerCase()] || null;
}

function findColumn(row, keywords) {
  for (const k of Object.keys(row)) {
    if (keywords.some(kw => k.toLowerCase().includes(kw))) return k;
  }
  return null;
}
...
function parseParticipants(data) {
  ...
  for (const row of data) {
    const idCol        = findColumn(row, ['id', 'n°']);
    const nomCol       = findColumn(row, ['nom2']);
    const prenomCol    = findColumn(row, ['prénom', 'prenom', 'first']);
    const challengeCol = findColumn(row, ['challenge', 'course', 'épreuve', 'évenement']);
    const emailCol     = findColumn(row, ['adresse e-mail professionnelle', 'email', 'e-mail', 'mail', 'courriel', 'adresse email']);
    const sexeCol      = findColumn(row, ['sexe']);
    const ageCol       = findColumn(row, ['âge']);
    const regionCol    = findColumn(row, ['région']);
    const societeCol   = findColumn(row, ['société']);

    const id = String(row[idCol] || '').trim();
    if (!id || id === 'Id') continue; // header ou ligne vide

    const nom          = String(row[nomCol]       || '').trim().toUpperCase();
    const prenom       = String(row[prenomCol]    || '').trim();
    const challengeRaw = String(row[challengeCol] || '').trim();
    const email        = String(row[emailCol]     || '').trim();
    const sexe         = String(row[sexeCol]      || '').trim();
    const age          = String(row[ageCol]       || '').trim();
    const region       = String(row[regionCol]    || '').trim();
    const societe      = String(row[societeCol]   || '').trim();

    if (!nom && !prenom) continue;
    if (!challengeRaw) continue;

    // Un participant peut avoir plusieurs défis (séparés par ";")
    const challenges = challengeRaw.split(';').map(c => c.trim()).filter(Boolean);

    for (const ch of challenges) {
      const cat = normalizeCategory(ch);
      if (!cat) continue;

      const assignKey = `${id}_${cat}`;

      // Numéro déjà attribué → on le réutilise
      if (assignments[assignKey]) {
        newDossards.push({ id, nom, prenom, cat, email, sexe, age, region, societe,
          number: assignments[assignKey], name: `${prenom} ${nom}` });
        continue;
      }

      // Attribution d'un nouveau numéro
      const catConf = CATS[cat];
      if (counters[cat] > catConf.end) {
        showToast(`⚠️ Plus de numéros disponibles pour « ${cat} » !`);
        continue;
      }

      const num = counters[cat]++;
      assignments[assignKey] = num;
      newDossards.push({ id, nom, prenom, cat, email, sexe, age, region, societe,
        number: num, name: `${prenom} ${nom}` });
    }
  }
  ...
  allDossards = newDossards;
  renderGrid();
  updateStats();
}

// ═══════════════════════════════════════════════════════════════════
// SECTION 5 — RENDU DE LA GRILLE
// ═══════════════════════════════════════════════════════════════════
...
function trameMissingHTML() {
  return `
    <div class="empty-state trame-waiting">
      <span class="trame-waiting-icon">🖼️</span>
      <h2>Trame non chargée</h2>
      <p>Charge d'abord la trame PNG avant de visualiser les dossards.</p>
      <p style="margin-top:6px;font-size:13px;">
        Besoin d'un point de départ ?
        <a href="#" onclick="downloadBlankTemplate();return false;" style="color:var(--accent);text-decoration:underline;">
          Télécharge la trame vierge annotée ↗
        </a>
      </p>

      <div class="trame-spec">
        <div class="trame-spec-row">
          <span class="trame-spec-label">Format</span>
          <span class="trame-spec-value">PNG, A4 paysage</span>
          <span class="trame-spec-tag">297 × 210 mm</span>
        </div>
        <div class="trame-spec-row">
          <span class="trame-spec-label">Résolution idéale</span>
          <span class="trame-spec-value">${TRAME_RES_IDEAL}</span>
          <span class="trame-spec-tag best">300 DPI — impression ✓</span>
        </div>
        <div class="trame-spec-row">
          <span class="trame-spec-label">Résolution minimum</span>
          <span class="trame-spec-value">${TRAME_RES_MIN}</span>
          <span class="trame-spec-tag min">~107 DPI — aperçu</span>
        </div>
        <div class="trame-spec-row">
          <span class="trame-spec-label">Ratio largeur/hauteur</span>
          <span class="trame-spec-value">${A4_LANDSCAPE_RATIO.toFixed(4)}</span>
          <span class="trame-spec-tag">tolérance ±${(TRAME_RATIO_TOLERANCE * 100).toFixed(0)} %</span>
        </div>
      </div>

      <button class="upload-btn" onclick="document.getElementById('trameUpload').click()">
        🖼️ Choisir la trame PNG
      </button>
      <button class="upload-btn" style="background:var(--surface2);border:1px solid var(--border);color:var(--text);margin-left:10px;"
              onclick="downloadBlankTemplate()">
        📐 Télécharger la trame vierge
      </button>
    </div>`;
}

// ── Rendu d'un dossard ───────────────────────────────────────────
// Layout minimaliste : seul le contenu dynamique est superposé à la trame.
// La trame porte le design (logos, nom de l'événement, couleurs…)
function renderDossard(d) {
  const cat    = CATS[d.cat];
  const num    = formatNumber(d.number);
  const hidden = currentFilter !== 'all' && d.cat !== currentFilter;

  return `
  <div class="dossard-wrapper${hidden ? ' hidden-print' : ''}"
       data-cat="${d.cat}"
       style="${hidden ? 'display:none' : ''}">

    <!-- Repères de découpe (impression uniquement) -->
    <div class="cut-marks-container">
      <div class="cut-line-h-top"></div>
      <div class="cut-line-h-bottom"></div>
      <div class="cut-line-v-left"></div>
      <div class="cut-line-v-right"></div>
      <div class="cut-corner tl">✂</div>
      <div class="cut-corner tr">✂</div>
      <div class="cut-corner bl">✂</div>
      <div class="cut-corner br">✂</div>
      <div class="cut-label top">✂ découper</div>
      <div class="cut-label bottom">✂ découper</div>
      <div class="cut-label left">✂ découper</div>
      <div class="cut-label right">✂ découper</div>
    </div>

    <div class="dossard ${cat.cssClass}">
      <!-- Trame de fond -->
      <img class="dossard-trame-bg" src="${trameSrc}" alt="">
...
// ── Rendu de la grille — APERÇU COMPACT (1 dossard démo par catégorie) ──
function renderGrid() {
  const grid = document.getElementById('grid');

  // 1. Trame obligatoire
  if (!trameSrc) {
    grid.innerHTML = trameMissingHTML();
    return;
  }

  // 2. Compter les dossards réels par catégorie
  const countByCat = {};
  Object.keys(CATS).forEach(cat => countByCat[cat] = 0);
  allDossards.forEach(d => { if (countByCat[d.cat] !== undefined) countByCat[d.cat]++; });

  // 3. Construire un dossard factice par catégorie active
  const demoDossards = currentConfig.categories.map(c => {
    const cat = CATS[c.name];
    if (!cat) return null;
    return {
      cat: c.name,
      number: cat.start,
      name: 'PRÉNOM NOM',
      count: countByCat[c.name] || 0,
    };
  }).filter(Boolean);

  // 4. Filtrer selon le filtre actif
  const visible = currentFilter === 'all'
    ? demoDossards
    : demoDossards.filter(d => d.cat === currentFilter);

  // 5. Rendu
  const infoText = `<div class="preview-info">
    <span class="preview-info-icon">👁️</span>
    Aperçu — un dossard type par catégorie. L'export génère les dossards personnalisés de chaque participant.
  </div>`;

  const cards = visible.map(d => {
    const rendered = renderDossard(d);
    const countLabel = d.count > 0
      ? `<div class="preview-counter"><span class="preview-counter-num">${d.count}</span> dossard${d.count > 1 ? 's' : ''} attribué${d.count > 1 ? 's' : ''}</div>`
      : `<div class="preview-counter preview-counter--empty">0 dossard attribué</div>`;
    return `<div class="preview-card">${rendered}${countLabel}</div>`;
  }).join('');

  grid.innerHTML = infoText + cards;
}
...
// ═══════════════════════════════════════════════════════════════════
// SECTION 8 — LISTE DES PARTICIPANTS
// ═══════════════════════════════════════════════════════════════════

let CAT_COLS = Object.keys(CATS);

function updateCatCols() {
  CAT_COLS = Object.keys(CATS);
}

// Regroupe les dossards par participant (clé: NOM|PRENOM|EMAIL)
function groupByParticipant() {
  const map = new Map();
  allDossards.forEach(d => {
    const key = d.id;  // ID unique du Forms — 1 inscription = 1 participant
    if (!map.has(key)) {
      map.set(key, {
        id: d.id, nom: d.nom, prenom: d.prenom, email: d.email,
        sexe: d.sexe, age: d.age, region: d.region, societe: d.societe,
        dossardsParCat: {},
      });
    }
    if (CAT_COLS.includes(d.cat)) {
      map.get(key).dossardsParCat[d.cat] = d.number;
    }
  });
...
function renderList(search = '') {
  ...
  if (!allDossards || allDossards.length === 0) {
    listContent.innerHTML = `
      <div class="empty-state">
        <h2>Aucun participant chargé</h2>
        <p>Importe ton fichier Excel pour voir la liste.</p>
      </div>`;
    return;
  }

  const term = search.trim().toLowerCase();
  let participants = groupByParticipant();

  if (term) {
    participants = participants.filter(p => {
      const base = `${p.prenom} ${p.nom} ${p.email}`.toLowerCase();
      if (base.includes(term)) return true;
      return CAT_COLS.some(cat => {
        const num = p.dossardsParCat[cat];
        return num && formatNumber(num).toLowerCase().includes(term);
      });
    });
    if (participants.length === 0) {
      listContent.innerHTML = `
        <div class="empty-state">
          ...
          <p>Aucun participant ne correspond à « ${search} ».</p>
        </div>`;
      return;
    }
  }
  ...
  let html = `
    <div class="list-section">
      <div class="list-section-header">
        <div class="list-section-title">Liste des participants</div>
        <div class="list-section-count">${participants.length} participant${participants.length > 1 ? 's' : ''}</div>
      </div>
      <table class="list-table">
        <thead>
          <tr>
            <th>Nom</th><th>Prénom</th><th>Sexe</th><th>Âge</th>
            <th>Région</th><th>Société</th>
            ${CAT_COLS.map(c => `<th>${c}</th>`).join('')}
          </tr>
        </thead>
        <tbody>`;
  ...
  participants.forEach(p => {
    html += `<tr>
      ...
      <td>${p.sexe||''}</td><td>${p.age||''}</td>
      <td>${p.region||''}</td><td>${p.societe||''}</td>`;
    CAT_COLS.forEach(cat => {
      const num = p.dossardsParCat[cat];
      html += `<td class="num-cell">${num ? formatNumber(num) : '—'}</td>`;
    });
    html += `</tr>`;
  });
...
function exportCSV() {
  ...
  const rows = [
    ['NOM','PRÉNOM','EMAIL','Sexe','Âge','Région','Société',
     ...CAT_COLS]
  ];
  participants.forEach(p => {
    const row = [p.nom, p.prenom, p.email, p.sexe, p.age, p.region, p.societe];
  ...
  const a    = Object.assign(document.createElement('a'), { href: url, download: `participants_${currentConfig.eventName.replace(/\s+/g, '_')}.csv` });
  a.click();
...
function exportCSVAnonymized() {
  ...
  participants.forEach(p => {
    const nomAnon    = (p.nom    ? p.nom[0].toUpperCase()    : '') + '*****';
  ...
  const a    = Object.assign(document.createElement('a'), {
    href: url,
    download: `participants_anonymises_${currentConfig.eventName.replace(/\s+/g, '_')}.csv`
  });
  a.click();
  URL.revokeObjectURL(url);
...
// Génère le HTML d'un dossard à taille fixe pour html2canvas
...
function buildDossardHTMLForExport(d) {
  ...
  const macBg   = getMacaronColor(d.cat);
  const macText = getMacaronTextColor(d.cat);
  ...
  ">
    ...
    ">
      <!-- Numéro -->
      <div style="
        font-size:520px; font-weight:900; line-height:0.82;
    ...
    <div style="
      position:absolute; bottom:60px; right:64px; z-index:2;
      display:flex; flex-direction:column;
    ...
    ">
      <span style="
        ...
        color:${macText}; letter-spacing:-1px;
      ">${cat.dist}</span>
      <span style="
        font-size:38px; font-weight:700;
        color:${macText}; opacity:.9; letter-spacing:3px;
      ">KM</span>
      <span style="
        font-size:30px; font-weight:700;
        color:${macText}; letter-spacing:2px;
...
// ═══════════════════════════════════════════════════════════════════
// GÉNÉRATEUR DE TRAME VIERGE ANNOTÉE
// ═══════════════════════════════════════════════════════════════════
// Génère un PNG A4 paysage @ 150 DPI (1748 × 1240 px) montrant
// l'emplacement exact de chaque zone de texte dynamique.

function downloadBlankTemplate() {
  const W = PNG_W;  // 1748 px
  const H = PNG_H;  // 1240 px
  ...
  // ── Fond principal ──────────────────────────────────────────────
  // Fond blanc cassé + motif de grille légère pour rappeler le format
  ctx.fillStyle = '#f5f5f0';
  ctx.fillRect(0, 0, W, H);

  // Motif grille tous les 50px
  ...
  ctx.fillText('ZONE LIBRE — VOTRE DESIGN ICI', W / 2, 70);
  ctx.font = '24px Arial';
  ctx.fillStyle = 'rgba(0,0,0,0.12)';
  ctx.fillText('Logos, nom de l\'événement, couleurs, photos…', W / 2, 110);
  ctx.fillText(`Format : ${W} × ${H} px  ·  A4 paysage ·  ratio ${A4_LANDSCAPE_RATIO.toFixed(4)}`, W / 2, 144);

  // ── Fonction helper : dessin d'une zone annotée ─────────────────
  function drawZone({ x, y, w, h, color, label, sublabel, fontSize = 28, dashed = true }) {
    // Rectangle tirets
    ctx.save();
    ctx.strokeStyle = color;
    ctx.lineWidth = 3;
    if (dashed) ctx.setLineDash([16, 8]);
    ...
    ctx.fillText(label, x + w / 2, y + h / 2 - (sublabel ? 20 : 0));
    if (sublabel) {
      ctx.font = `${Math.round(fontSize * 0.72)}px Arial`;
      ctx.fillStyle = color.replace(')', ', 0.75)').replace('rgb', 'rgba');
      ctx.fillText(sublabel, x + w / 2, y + h / 2 + Math.round(fontSize * 0.65));
    }
    ctx.restore();

    // Dimensions en coin bas-droit
    ctx.save();
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    ctx.font = '18px monospace';
    ctx.textAlign = 'right';
    ctx.textBaseline = 'bottom';
    ctx.fillText(`${w} × ${h} px`, x + w - 8, y + h - 6);
    ctx.restore();
  }

  // Marges utilisées dans le layout réel (voir buildDossardHTMLForExport)
  const PAD_X = 80;   // padding-left
  const PAD_B = 80;   // padding-bottom

  // ── Zone : NUMÉRO DE DOSSARD ────────────────────────────────────
  // Le numéro fait ~520px de font-size, line-height 0.82 → hauteur ~426px
  // Il est aligné en bas-gauche, au-dessus du trait
  const numH  = 430;
  const numW  = 800;
  const numY  = H - PAD_B - 24 - 6 - 36 - 100 - numH;  // approx
  drawZone({
    x: PAD_X, y: numY,
    w: numW,  h: numH,
    color: 'rgb(220, 50, 50)',
    label: '0001',
    sublabel: 'Numéro de dossard  (4 chiffres)',
    fontSize: 100,
    dashed: true,
  });

  // Trait sous le numéro
  const traitY = numY + numH + 18;
  ctx.fillStyle = 'rgba(220,50,50,0.35)';
  ctx.fillRect(PAD_X, traitY, 480, 6);
  ctx.fillStyle = 'rgba(220,50,50,0.5)';
  ctx.font = '20px Arial';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillText('← trait séparateur automatique', PAD_X + 500, traitY - 2);

  // ── Zone : NOM DU PARTICIPANT ───────────────────────────────────
  const nomY = traitY + 6 + 16;
  const nomW = W - PAD_X - 280 - 30; // laisse la place au badge
  const nomH = 120;
  drawZone({
    x: PAD_X, y: nomY,
    w: nomW,  h: nomH,
    ...
    label: 'PRÉNOM NOM',
    sublabel: 'Nom du participant  (majuscules automatiques)',
    fontSize: 36,
    dashed: true,
  });

  // ── Zone : BADGE TYPE/DISTANCE ──────────────────────────────────
  const badgeW = 240;
  const badgeH = 200;
  const badgeX = W - 64 - badgeW;
  const badgeY = H - 60 - badgeH;

  // Rectangle arrondi
  const r = 12;
  ctx.save();
  ctx.strokeStyle = 'rgb(40, 160, 80)';
  ctx.lineWidth = 3;
  ...
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillStyle = 'rgba(40, 160, 80, 0.08)';
  ctx.fill();
  ctx.restore();
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('5 KM', badgeX + badgeW/2, badgeY + badgeH/2 - 24);
  ctx.font = 'bold 28px Arial';
  ctx.fillText('COURSE', badgeX + badgeW/2, badgeY + badgeH/2 + 24);
  ...
  ctx.textAlign = 'right';
  ctx.textBaseline = 'top';
  ctx.fillText('← badge dist./type', badgeX - 10, badgeY + 80);
  ...
  // Pastilles légende
  const items = [
    { color: 'rgb(220,50,50)',   label: 'Numéro de dossard' },
    { color: 'rgb(30,100,200)',  label: 'Nom du participant' },
    { color: 'rgb(40,160,80)',   label: 'Badge type/distance' },
  ];
  const totalLW = items.length * 280;
  let lx = (W - totalLW) / 2;
  for (const item of items) {
    ctx.fillStyle = item.color;
    ctx.beginPath(); ctx.arc(lx + 12, legendY - 10, 10, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = 'rgba(0,0,0,0.4)';
    ctx.font = '20px Arial';
    ctx.textAlign = 'left';
    ctx.textBaseline = 'bottom';
    ctx.fillText(item.label, lx + 30, legendY);
    lx += 280;
  }
  ...
  canvas.toBlob(blob => {
    ...
    const a   = Object.assign(document.createElement('a'), {
      href:     url,
      download: 'trame_vierge_dossard_A4_paysage.png',
    });
    a.click();
    URL.revokeObjectURL(url);
    showToast('✅ Trame vierge téléchargée (1748 × 1240 px, A4 paysage)');
  }, 'image/png');
}
...
function saveDossardsPNG() {
  ...
  if (!('showDirectoryPicker' in window)) {
    showToast('❌ Navigateur non compatible — utilise Chrome ou Edge.');
    return;
  }
...
async function _runExportPNG(idRange, explicitList = null, pushToGitHub = false) {
  ...
  const modalSub      = document.getElementById('exportModalSub');
  document.getElementById('exportModalTitle').textContent = `Génération des PNG…`;
  overlay.classList.remove('hidden');

  // Fichiers déjà présents (pour skip)
  const existingFiles = new Set();
  try {
    for await (const [name] of dirHandle.entries()) existingFiles.add(name);
  } catch {}
  ...
  for (const d of toGenerate) {
    ...
    if (existingFiles.has(filename)) {
      ...
      progressLabel.innerHTML   = `<strong>${done}</strong> / ${total} &nbsp;·&nbsp; ⏭️ ${skipped} ignoré(s)`;
      modalSub.textContent      = `⏭️ ${filename} déjà existant, ignoré`;
      continue;
    }

    modalSub.textContent = `Génération de ${filename}…`;

    try {
      renderContainer.innerHTML = buildDossardHTMLForExport(d);

      // Attente de toutes les images
      await Promise.all(
        [...renderContainer.querySelectorAll('img')].map(img =>
      ...
      const canvas = await html2canvas(renderContainer.firstElementChild, {
        scale: 1,              // le conteneur est déjà à la bonne taille
        useCORS: true,
      ...
      progressLabel.innerHTML  = `<strong>${done}</strong> / ${total}` +
        (skipped ? ` &nbsp;·&nbsp; ⏭️ ${skipped} ignoré(s)` : '');

    } catch (err) {
      console.error('Erreur export', filename, err);
      modalSub.textContent = `❌ Erreur : ${err.message || err}`;
      errors++; done++;
  ...
  const newFiles = done - skipped - errors;
  let msg = `✅ ${newFiles} PNG(s) enregistré(s)`;
...
// ═══════════════════════════════════════════════════════════════════
// SECTION 10 — UTILITAIRES COULEURS MACARON
// ═══════════════════════════════════════════════════════════════════

function getMacaronColor(cat) {
  if (CATS[cat] && CATS[cat].color) return CATS[cat].color;
  return '#f773b4';
}

function getMacaronTextColor(cat) {
  if (CATS[cat] && CATS[cat].color) {
    return isLightColor(CATS[cat].color) ? 'rgba(0,0,0,0.75)' : 'white';
  }
  return 'white';
}
...
// ═══════════════════════════════════════════════════════════════════
// SECTION 12 — ONGLET STATISTIQUES
// ═══════════════════════════════════════════════════════════════════

/* ─────────────────────────────────────────────
   COULEURS CATÉGORIES (pour graphiques)
───────────────────────────────────────────── */
function getCatColors() {
  const colors = {};
  Object.keys(CATS).forEach(name => {
    colors[name] = CATS[name].color;
  });
  return colors;
}
...
/* ─────────────────────────────────────────────
   ...
   ViewBox: 0 0 470 540
───────────────────────────────────────────── */
const FRANCE_REGIONS_MAP = [
  { name: 'Auvergne-Rhône-Alpes', id: 'ara', lx: 402, ly: 350,
  ...
  { name: 'Occitanie', id: 'occ', lx: 302, ly: 447,
...
function normalizeRegion(raw) {
  if (!raw) return null;
  const keyRaw = raw.trim().toLowerCase()
    .replace(/[\u2018\u2019\u201A\u201B]/g, "'");  // smart quotes → straight
  const keyStripped = keyRaw
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // strip accents
    .replace(/\s+/g, ' ');
  return REGION_ALIASES[keyRaw] || REGION_ALIASES[keyStripped] || null;
}
...
function renderStats() {
  ...
  renderKPIs(s.totalDossards, s.totalParticipants, s.byCat, s.bySexe, s.ageMoy, s.totalKm);
...
/* ─────────────────────────────────────────────
   KPI CARDS
───────────────────────────────────────────── */
function renderKPIs(totalDossards, totalPart, byCat, bySexe, ageMoy, totalKm) {
  ...
  document.getElementById('statsKpis').innerHTML = `
    <div class="kpi-card kpi-card--pink">
      ...
      <div class="kpi-value">${totalDossards}</div>
      <div class="kpi-label">Dossards générés</div>
    </div>
    <div class="kpi-card kpi-card--blue">
      <div class="kpi-icon">👥</div>
      <div class="kpi-value">${totalPart}</div>
      <div class="kpi-label">Participants uniques</div>
    </div>
    <div class="kpi-card kpi-card--green">
      <div class="kpi-icon">🚶</div>
      <div class="kpi-value">${totalMarches}</div>
      <div class="kpi-label">Marches</div>
      <div class="kpi-sub">${totalCourses} course${totalCourses > 1 ? 's' : ''}</div>
    </div>
    ...
    <div class="kpi-card kpi-card--purple">
      <div class="kpi-icon">🎂</div>
      <div class="kpi-value">${ageMoy !== null ? ageMoy : '—'}</div>
      <div class="kpi-label">Âge moyen</div>
      <div class="kpi-sub">ans</div>
    </div>
...
function renderCatBars(byCat) {
  ...
  const html = Object.entries(byCat).map(([cat, count]) => {
    ...
    const color = getCatColors()[cat] || '#888';
    return `
      <div class="cat-bar-item">
        <div class="cat-bar-header">
          <span class="cat-bar-name">${cat}</span>
          <span class="cat-bar-count">${count} · ${pct} %</span>
        </div>
...
function renderSexeDonut(bySexe, total) {
  ...
  entries.forEach(([label, count]) => {
    ...
    const color = SEXE_COLORS[label] || '#888';
    slices += `<path d="M${lx1},${ly1} A${R},${R} 0 ${large},1 ${lx2},${ly2} L${sx2},${sy2} A${r},${r} 0 ${large},0 ${sx1},${sy1} Z"
  ...
  const legendItems = entries.map(([label, count]) => {
    ...
    return `<div class="sexe-legend-item">
      <div class="sexe-dot" style="background:${color}"></div>
      <span>${label}</span>
      <strong style="color:var(--text);margin-left:4px">${pct} %</strong>
    </div>`;
  }).join('');
...
/* ─────────────────────────────────────────────
   CARTE DE FRANCE SVG
───────────────────────────────────────────── */
function renderFranceMap(byRegion) {
  const body = document.getElementById('chartRegionsBody');
  const maxCount = Math.max(...Object.values(byRegion), 1);

  // Palette : from surface2 (no data) → accent (max)
  function regionFill(name) {
    const count = byRegion[name] || 0;
    if (count === 0) return 'var(--surface2)';
    const t = count / maxCount;
    // Interpolate from #2d3149 → #f773b4
    const r = Math.round(45 + t * (247 - 45));
    const g = Math.round(49 + t * (115 - 49));
    const b = Math.round(73 + t * (180 - 73));
    return `rgb(${r},${g},${b})`;
  }

  const paths = FRANCE_REGIONS_MAP.map(reg => {
    const count = byRegion[reg.name] || 0;
    const fill = regionFill(reg.name);
    return `
      <path class="france-region" d="${reg.path}" fill="${fill}"
               data-region="${reg.name}" data-count="${count}">
        <title>${reg.name} : ${count} participant${count > 1 ? 's' : ''}</title>
      </path>
      ${count > 0 ? `<text class="france-region-label" x="${reg.lx}" y="${reg.ly}"
            style="fill:${count/maxCount > 0.4 ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.6)'};font-size:14px"
            >${count}</text>` : ''}`;
  }).join('');
  ...
  const legendItems = sortedRegions.map(([name, count]) => {
    ...
    return `<div class="france-legend-item">
      <div class="france-legend-dot" style="background:${fill}"></div>
      <span>${shortName}</span>
      <span class="france-legend-count">${count}</span>
    </div>`;
  }).join('');

  const noDataMsg = Object.keys(byRegion).length === 0
    ? '<p class="france-no-data" style="margin-top:8px">Aucune donnée région dans le fichier.</p>'
    : '';

  body.innerHTML = `
    <div class="france-map-wrap">
      <div class="france-map-svg">
        <svg viewBox="15 -1 598 586" xmlns="http://www.w3.org/2000/svg">
          ${paths}
        </svg>
      </div>
      <div class="france-legend">
        ${noDataMsg}
        ${legendItems}
        ${Object.keys(byRegion).length === 0 ? '' : `<p style="font-size:11px;color:var(--muted);margin-top:6px">
          Régions non reconnues comptabilisées séparément.</p>`}
      </div>
    </div>`;
}
...
async function captureStats() {
  ...
  try {
    ...
    canvas.toBlob(blob => {
      ...
      const a = Object.assign(document.createElement('a'), {
        href: url,
        download: `stats_${currentConfig.eventName.replace(/\s+/g, '_')}.png`,
      });
      a.click();
      URL.revokeObjectURL(url);
    ...
    showToast('❌ Erreur lors de la capture : ' + err.message);
  } finally {
    btn.disabled = false;
    btn.innerHTML = '📸 Capturer le tableau de bord';
  }
}

// ─────────────────────────────────────────────
// PATCH switchTab : déclenche renderStats / renderDoublons à chaque activation
// ─────────────────────────────────────────────
const _switchTabOrig = switchTab;
switchTab = function(name, btn) {
  _switchTabOrig(name, btn);
  if (name === 'stats') renderStats();
  if (name === 'doublons') renderDoublons();
};

// ─────────────────────────────────────────────
// PATCH updateStats : met à jour les compteurs onglets stats + doublons
// ─────────────────────────────────────────────
...
// type 'nonpro' : d = un seul participant
// type 'doublon' : d = tableau de participants du groupe
function buildMailtoAnomalie(type, d) {
  const encode = s => encodeURIComponent(s).replace(/%0A/g, '%0D%0A');
  const FORM_URL = 'https://forms.office.com/e/kS7ijtKDZR';
  ...
  if (type === 'nonpro') {
    ...
    subject = 'Challenge Connecté 2026 — Validation de ton inscription';
    body =
      `Bonjour ${prenom},\n\n` +
      `On est vraiment ravis de te compter parmi les participants au Challenge Connecté 2026 !\n\n` +
      `On a constaté que ton inscription a été réalisée avec une adresse email personnelle (${to}), ` +
      `alors que l'événement est ouvert aux collaborateurs SNCF et de ses filiales avec une adresse email professionnelle.\n\n` +
      `Pour valider définitivement ta participation, il te suffit de refaire ton inscription avec ton adresse email professionnelle :\n` +
      `${FORM_URL}\n\n` +
      `Toutes nos excuses pour ce désagrément — et encore un immense merci pour ton enthousiasme !\n\n` +
      `Have Fun Have Run !\n` +
      `L'équipe SoRunning SNCF`;
  } else {
    const entries = d;
    const emails  = [...new Set(entries.map(e => (e.email || '').trim()).filter(Boolean))];
    ...
    body =
      ...
      `Lien d'inscription :\n` +
      `${FORM_URL}\n\n` +
      `Pas d'inquiétude, ça prend deux minutes. N'hésite pas à répondre à ce message si tu as la moindre question.\n\n` +
      `Have Fun Have Run !\n` +
      `L'équipe SoRunning SNCF`;
  }

  return `mailto:${to}?subject=${encode(subject)}&bcc=${encodeURIComponent(ANOMALIE_BCC)}&body=${encode(body)}`;
}
...
function detectDuplicates() {
  ...
  const byEmail    = new Map();  // même email
  const byName     = new Map();  // même nom + prénom (casse insensible)
  const byIdUnique = new Map();  // dédoublonne par ID pour accéder aux données

  allDossards.forEach(d => {
    if (!byIdUnique.has(d.id)) byIdUnique.set(d.id, d);
  });

  const participants = Array.from(byIdUnique.values());

  participants.forEach(d => {
    const email = (d.email || '').toLowerCase().trim();
    const name  = `${(d.nom || '').toUpperCase().trim()}|${(d.prenom || '').toUpperCase().trim()}`;

    if (email) {
      if (!byEmail.has(email)) byEmail.set(email, []);
      byEmail.set(email, [...byEmail.get(email), d]);
    }

    if (name !== '|') {
      if (!byName.has(name)) byName.set(name, []);
      byName.set(name, [...byName.get(name), d]);
    }
  });

  // 1) Doublons exacts : même email ET même nom+prénom
  const exact = [];
  byEmail.forEach((entries, email) => {
    if (entries.length < 2) return;
    // Vérifie si au moins 2 ont le même nom
    const nameGroups = new Map();
    entries.forEach(e => {
      const k = `${(e.nom||'').toUpperCase()}|${(e.prenom||'').toUpperCase()}`;
      if (!nameGroups.has(k)) nameGroups.set(k, []);
      nameGroups.get(k).push(e);
    });
    nameGroups.forEach((group, _) => {
      if (group.length >= 2) exact.push({ reason: 'Même nom, prénom et email', entries: group });
    });
  });

  // 2) Même nom+prénom mais email différent (possible erreur de saisie)
  const exactIds = new Set();
  exact.forEach(g => g.entries.forEach(e => exactIds.add(e.id)));

  const sameName = [];
  byName.forEach((entries, name) => {
    if (entries.length < 2) return;
    // Exclure ceux déjà dans "exact"
    const filtered = entries.filter(e => !exactIds.has(e.id));
    if (filtered.length < 2) return;
    // Vérifier que les emails diffèrent (sinon c'est un exact)
    const emails = new Set(filtered.map(e => (e.email||'').toLowerCase().trim()));
    if (emails.size > 1) {
      sameName.push({ reason: 'Même nom et prénom, emails différents', entries: filtered });
    }
  });

  // 3) Même email mais noms différents (compte partagé ?)
  const sameEmail = [];
  byEmail.forEach((entries, email) => {
    if (entries.length < 2) return;
    const names = new Set(entries.map(e => `${(e.nom||'').toUpperCase()}|${(e.prenom||'').toUpperCase()}`));
    if (names.size > 1) {
      sameEmail.push({ reason: 'Même email, noms différents', entries });
    }
  });

  return { exact, sameName, sameEmail };
}

function renderDoublons() {
  const container = document.getElementById('doublonsContent');
  if (!container) return;
  ...
  if (!allDossards || allDossards.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        <h2>Aucun participant chargé</h2>
        <p>Importe ton fichier Excel pour lancer la détection.</p>
      </div>`;
    return;
  }
  ...
  function renderGroup(group, severity) {
    const sevClass = severity === 'high' ? 'dupe-high' : severity === 'medium' ? 'dupe-medium' : 'dupe-low';
    const sevLabel = severity === 'high' ? '🔴 Critique' : severity === 'medium' ? '🟡 Attention' : '🔵 Info';

    const mailto = buildMailtoAnomalie('doublon', group.entries);
    ...
    return `
      <div class="dupe-group ${sevClass}">
        <div class="dupe-group-header">
          <span class="dupe-sev">${sevLabel}</span>
...
// ═══════════════════════════════════════════════════════════════════
// SECTION 13 — PARAMÈTRES (Feature 3)
// ═══════════════════════════════════════════════════════════════════
...
function renderSettingsPanel() {
  ...
  let html = `<div class="settings-cat-header">
    <span>Nom</span><span>Type</span><span>Distance</span><span>Début</span><span>Fin</span><span>Couleur</span><span></span>
  </div>`;

  currentConfig.categories.forEach((c, i) => {
    html += `<div class="settings-cat-row" data-index="${i}">
      <input type="text" value="${c.name}" data-field="name" onchange="updateSettingsCat(${i}, 'name', this.value)">
      <select data-field="type" onchange="updateSettingsCat(${i}, 'type', this.value)">
        <option value="Course" ${c.type === 'Course' ? 'selected' : ''}>Course</option>
        <option value="Marche" ${c.type === 'Marche' ? 'selected' : ''}>Marche</option>
      </select>
      <input type="text" value="${c.distance}" data-field="distance" onchange="updateSettingsCat(${i}, 'distance', this.value)">
      <input type="number" value="${c.start}" data-field="start" onchange="updateSettingsCat(${i}, 'start', parseInt(this.value))">
      <input type="number" value="${c.end}" data-field="end" onchange="updateSettingsCat(${i}, 'end', parseInt(this.value))">
      <input type="color" value="${c.color}" data-field="color" onchange="updateSettingsCat(${i}, 'color', this.value)">
      <button class="btn-del-cat" onclick="deleteSettingsCat(${i})" title="Supprimer">✕</button>
    </div>`;
  });

  list.innerHTML = html;
  document.getElementById('settingsWarning').style.display = currentConfig.categories.length > 0 ? 'block' : 'none';
}

function updateSettingsCat(index, field, value) {
  if (index >= 0 && index < currentConfig.categories.length) {
    currentConfig.categories[index][field] = value;
  }
}

function addSettingsCategory() {
  const maxEnd = currentConfig.categories.reduce((m, c) => Math.max(m, c.end || 0), 0);
...
function deleteSettingsCat(index) {
  if (currentConfig.categories.length <= 1) {
    showToast('⚠️ Il faut au moins une catégorie.');
    return;
  }
  const name = currentConfig.categories[index].name;
...
function saveSettings() {
  // Update event name
  const nameInput = document.getElementById('settingsEventName');
  if (nameInput) currentConfig.eventName = nameInput.value.trim() || DEFAULT_CONFIG.eventName;

  // Validate: no overlapping ranges
  for (let i = 0; i < currentConfig.categories.length; i++) {
    const a = currentConfig.categories[i];
    if (a.start > a.end) {
      showToast(`⚠️ « ${a.name} » : la plage de début (${a.start}) est supérieure à la fin (${a.end}).`);
      return;
    }
    for (let j = i + 1; j < currentConfig.categories.length; j++) {
      const b = currentConfig.categories[j];
      if (a.start <= b.end && b.start <= a.end) {
        showToast(`⚠️ Les plages de « ${a.name} » et « ${b.name} » se chevauchent.`);
        return;
      }
    }
  }
...
// On DOMContentLoaded, apply config and render settings
document.addEventListener('DOMContentLoaded', () => {
  applyConfig();
  renderSettingsPanel();
});

// Patch switchTab to also handle settings
const _switchTabPrev = switchTab;
switchTab = function(name, btn) {
  _switchTabPrev(name, btn);
  if (name === 'settings') renderSettingsPanel();
};

// Patch: show tabsNav after file load (already visible, but ensure data tabs work)
const _processExcelFileOrig = processExcelFile;
// No need to patch — tabsNav is now always visible
